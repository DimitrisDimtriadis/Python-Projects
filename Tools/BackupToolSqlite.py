...
def main(pathForDB, filesLimitationPerDir=50):
    ...
    dbBackupPath = ut.regulatePathToSupportOS(dbLocatedPath + "\\" + backupDir)    
...
def extractNameFromPath(mPath, needExtension=False, needWholePath=False):
    ...
    if needWholePath:
        ...
        return ut.regulatePathToSupportOS(pathDBLocated)
    else:
...
if __name__ == "__main__":
    ...
    if numOfArg == 2:
        ...
        if ut.checkIfFileExists(ut.checkIfFileExists(sys.argv[1])):
            main(ut.regulatePathToSupportOS(sys.argv[1]))
        else:
